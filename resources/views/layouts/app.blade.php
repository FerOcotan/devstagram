  ...
  <header class="p-5 border-b border-gray-200 bg-white shadow">
    ...
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="flex gap-2 items-center">
          ...
          rounded text-sm uppercase font-bold">
          ...
          <a class="font-bold uppercase text-gray-600 text-sm"  href="{{ route('post.index',Auth::user()->username) }}">HI: <span class="font-normal">{{ auth()->user()->username }}</span></a>
          <form method="POST"  action="{{ route('logout') }}">
            @csrf
            <button type="submit"
          class="font-bold uppercase text-gray-600 text-sm" href="{{ route('logout') }}">Logout</button>
